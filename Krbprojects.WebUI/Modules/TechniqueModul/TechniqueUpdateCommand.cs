...
{
    public class TechniqueUpdateCommand : TechniqueViewModel, IRequest<int>
    {
        public class TechniqueUpdateCommandHandler : IRequestHandler<TechniqueUpdateCommand, int>
        {
            readonly KrbProjectsBaseDbContext db;
            readonly IActionContextAccessor ctx;
            readonly IWebHostEnvironment env;
            public TechniqueUpdateCommandHandler(KrbProjectsBaseDbContext db, IActionContextAccessor ctx, IWebHostEnvironment env)
            {
                this.ctx = ctx;
                this.db = db;
                this.env = env;
            }
            public async Task<int> Handle(TechniqueUpdateCommand request, CancellationToken cancellationToken)
            {
                ...
                var entity = await db.Techniques.FirstOrDefaultAsync(b => b.Id == request.Id);
